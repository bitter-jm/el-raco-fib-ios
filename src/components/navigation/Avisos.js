...
import { Text, View, TouchableOpacity, ScrollView, Dimensions, FlatList, PermissionsAndroid, ActivityIndicator, Platform, Alert, Linking } from 'react-native';
        ...
        conjAdj.push(
          <TouchableOpacity  key={adj.nom} onPress={() => {
            this.pedirPermisosWrite().then(() => updateToken.bind(this)()).then((newToken) => {
              ...
              // Puede que haya que usar otro recurso para descargar el doc
              Linking.openURL(adj.url, {Authorization:`Bearer ${newToken}`});
